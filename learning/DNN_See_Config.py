...
def train(epoch):
    ...
    # now that the epoch is completed plot the acc
...
def nn_test():
    ...
    print("The average accuracy is: {} %".format(100.0 * test_correct / (float(total_examples))))
    return 100.0 * test_correct / (float(total_examples))


def classifyOneImage(file, net, weights, model_num,relativePathtoDNN,relativePathToImage):
    if model_num == 0:
...
def classifythis():
    ...
    net.eval()
    #loading classes
    target = ['BottomCenter','BottomLeft','BottomRight','MiddleLeft','MiddleRight','TopCenter','TopLeft', 'TopRight']
    #creating var that will contain the result
    df = pd.DataFrame(columns=["file", "prediction","proba_class0","proba_class1","proba_class2","proba_class4","proba_class5","proba_class6","proba_class7","proba_class8"])
    i = 0
    number_of_file = len(os.listdir(to_classify_data))
    for file in os.listdir(to_classify_data):
        if file.lower().endswith(".jpg"):
            result, output = classifyOneImage (file,net,weights,model_num,"",None)
            print("the class with the highest proba :")
            print (result)
            df = df.append({"file": file,"prediction":  result,"proba_class0": output[0][0],"proba_class1": output[0][1],"proba_class2":  output[0][2],"proba_class4": output[0][3],"proba_class5": output[0][4],"proba_class6": output[0][5],"proba_class7": output[0][6],"proba_class8": output[0][7]}, ignore_index=True)
            i = i + 1
...
if __name__== '__main__':
    ...
    if action.strip().lower() == "show_weights":
        print("You want to see the weights")
        torch.set_printoptions(profile="full")
        show_Weights("yes")
    else:
        if action.strip().lower() == "classify" :
            print("You want to classify the images using exiting trained model")
            classifythis()
        else:
